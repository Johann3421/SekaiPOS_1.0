...
{
    public class InventoryFormFixed : Form
    {
        ...
        public InventoryFormFixed(DatabaseHelper database)
        {
            if (database == null)
        ...
        private void LoadProducts()
        {
            try
            {
                if (db == null)
                {
                    MessageBox.Show("Error: Base de datos no inicializada", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    return;
                }
        ...
        private void FilterProducts()
        {
            try
            {
                ...
                // Re-configure columns after filtering
                if (dgvProducts.Columns.Count > 0)
                {
                    if (dgvProducts.Columns.Contains("Price"))
                        dgvProducts.Columns["Price"].DefaultCellStyle.Format = "C2";
        ...
        private void BtnDelete_Click(object? sender, EventArgs e)
        {
            try
            {
                if (dgvProducts.SelectedRows.Count > 0)
                {
                    var result = MessageBox.Show("¿Estás seguro de eliminar este producto?", "Confirmar Eliminación", MessageBoxButtons.YesNo, MessageBoxIcon.Question);
