...
namespace SekaiPOS_1._0
{
    public class MainDashboard : Form
    {
        ...
        private void InitializeComponent()
        {
            ...
            {
                ActivateButton(s);
                iconCurrentForm.IconChar = IconChar.FileInvoiceDollar;
                lblTitle.Text = "Reportes";
                MessageBox.Show("Módulo de reportes en desarrollo", "Info", MessageBoxButtons.OK, MessageBoxIcon.Information);
            };

            btnSettings.Click += (s, e) =>
            {
                ActivateButton(s);
                iconCurrentForm.IconChar = IconChar.Gear;
                lblTitle.Text = "Configuración";
                MessageBox.Show("Módulo de configuración en desarrollo", "Info", MessageBoxButtons.OK, MessageBoxIcon.Information);
            };
        ...
        private void OpenDashboardHome()
        {
            ...
            var card1 = CreateDashboardCard(IconChar.BoxesStacked, "Productos en Stock", totalProducts.ToString(), Color.FromArgb(0, 120, 212), 40, 100);
            ...
            var card4 = CreateDashboardCard(IconChar.ChartLine, "Promedio Venta", totalSales > 0 ? (todaySales / totalSales).ToString("C2") : "$0.00", Color.FromArgb(156, 39, 176), 880, 100);
        ...
        {
            var inventoryForm = new InventoryForm(db);
            OpenChildForm(inventoryForm, IconChar.BoxesStacked, "Inventario");
        }

        private void OpenSalesForm()
        {
            var salesForm = new SalesForm(db);
            OpenChildForm(salesForm, IconChar.CashRegister, "Ventas");
        }
